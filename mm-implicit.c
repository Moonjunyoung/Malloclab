...
#define PACK(size,alloc) ((size)|(alloc)) //header와footer에 값을넣기위해
 ...
 */
int mm_init(void) {
	if((heap_listp = mem_sbrk(4*WSIZE))== NULL) //초기 empty heap생성여기서
		return -1;  // empty heap 늘림 

	PUT(heap_listp,0);//healp_listp가 가리키는곳에 0을 넣음
	PUT(heap_listp+WSIZE,PACK(OVERHEAD,1)); //heap_listp+4 위치에9들어감1은 block의 할당유무를 알려주는 word
	PUT(heap_listp+DSIZE, PACK(OVERHEAD,1));//heap_list+8 위치에 위와 동일
	PUT(heap_listp+WSIZE+DSIZE,PACK(0,1)); //heap_list+12 위치에 1 위와동일
	heap_listp+=DSIZE; //heap_listp+8위치로 heap_listp 가이동
...
void free (void *bp) {
   ...
   coalesce(bp); //주위에 빈 블로기 있을시 병합한다.
...
static void *extend_heap(size_t words){
	char *bp; //char 형 block pointer선언
	size_t size; // unsigind int 64비트 타입 size 선언

	size= (words%2)?(words+1)*WSIZE:words*WSIZE; //들어오는 words를 size로변환시켜줌 4096이들어오면 1024로 바꿔짐
	if((long)(bp=mem_sbrk(size))==-1) //위에서 정한size로 mem_sbrk를통해힙 영역이 늘어남 bp는 oldbrk의 주소영역을 가리키고있음 mem_brk는 맨아래
		return NULL;

	PUT(HDRP(bp),PACK(size,0));//bp의 header에 size와 워드에 0저장 
